          ...
          {city && (
            <div className="space-y-3 p-3 bg-muted/30 rounded-lg">
              ...
              <div className="flex items-center gap-3">
                <Label className="text-sm text-muted-foreground whitespace-nowrap">Scheduled Date</Label>
                <Input
                  type="date"
                  defaultValue={city.scheduledDate ? new Date(city.scheduledDate).toISOString().split('T')[0] : ""}
                  key={city.scheduledDate}
                  className="bg-background text-sm w-44"
                  data-testid="input-scheduled-date"
                  onBlur={(e) => {
                    const val = e.target.value;
                    if (val) {
                      updateCityMutation.mutate({ id: city.id, scheduledDate: val });
                    }
                  }}
                />
                <span className="text-xs text-muted-foreground">Auto-publishes at 12am EST on this date</span>
              </div>
            </div>
          )}
